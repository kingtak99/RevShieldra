...
<script>
    ...
    async function postChatbot(payload) {
        const response = await fetch(chatbotEndpoint, {
            ...
            body: JSON.stringify({
                ...
                language: currentLang || @json(app()->getLocale() === 'ar' ? 'ar' : 'en'),
                ...payload,
            }),
        });
        ...
        if (!response.ok || data.error) {
            throw new Error(data.reply || data.message || (currentLang === 'ar' ? 'تعذر تنفيذ الطلب. جرّب مرة ثانية.' : 'Request failed. Please try again.'));
        }
    ...
    function sendUserMessage() {
        ...
        // Always send message to server - let it handle smart routing
        postChatbot({
            action: 'chat',
            message: message,
        })
        .then(data => {
            // If server detected a flow match, navigate to it
            if (data.message_type === 'branch_list' || data.message_type === 'branch_answer') {
                ...
                if (data.branches) {
                    ...
                    showBranchMenu(buttons, data.flow_key);
                }
                
                if (data.sub_options) {
                    showSubOptions(data.sub_options, data.flow_key, data.branch_key);
                }
            } else {
    ...
    function showRootMenu() {
        ...
        .catch(error => {
            console.error('Root menu error:', error);
